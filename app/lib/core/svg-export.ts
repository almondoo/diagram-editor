...
import { getEdgePoints, getShapePath, buildEdgePath } from "./geometry";
...
export function generateExportSVG(parsed: ParseResult): string | null {
  ...
  edges.forEach((edge, i) => {
    ...
    const { pathD, labelX, labelY } = buildEdgePath(from, to, edge.curve, edge.bendX, edge.bendY);
  ...
  nodes.forEach((node) => {
    ...
    const ty = y + h / 2;
    const truncatedLabel = escapeXml(label.length > 18 ? `${label.slice(0, 17)}…` : label);
    if (icon) {
      const iconDir = icon.split(".")[0];
      const iconSize = 36;
      const iconX = x + w / 2 - iconSize / 2;
      const iconY = ty - iconSize / 2 - 4;
      svgContent += `<image href="icons/${iconDir}/${escapeXml(icon)}.svg" x="${iconX}" y="${iconY}" width="${iconSize}" height="${iconSize}"/>\n`;
      svgContent += `<text x="${x + w / 2}" y="${iconY + iconSize + 13}" text-anchor="middle" dominant-baseline="middle" fill="${escapeXml(textColor)}" font-size="13" font-family="system-ui, sans-serif" font-weight="500">${truncatedLabel}</text>\n`;
    } else {
      svgContent += `<text x="${x + w / 2}" y="${ty + 1}" text-anchor="middle" dominant-baseline="middle" fill="${escapeXml(textColor)}" font-size="13" font-family="system-ui, sans-serif" font-weight="500">${truncatedLabel}</text>\n`;
    }
  });
